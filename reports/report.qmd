...
SELECT
  c.`Region`,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'High income' THEN num_countries ELSE 0 END) * 100.0 / g.total_countries, 2) AS `High`,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Upper middle income' THEN num_countries ELSE 0 END) * 100.0 / g.total_countries, 2) AS `Upper Middle`,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Lower middle income' THEN num_countries ELSE 0 END) * 100.0 / g.total_countries, 2) AS `Lower Middle`,
  ROUND(SUM(CASE WHEN c.`Income Group` = 'Low income' THEN num_countries ELSE 0 END) * 100.0 / g.total_countries, 2) AS `Low`,
  ROUND(SUM(CASE WHEN c.`Income Group` IS NULL THEN num_countries ELSE 0 END) * 100.0 / g.total_countries, 2) AS `Other`
FROM
  counts c
CROSS JOIN
  grand_total g
