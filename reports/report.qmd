...
WITH income_counts AS (
  SELECT 
    CASE 
      WHEN `Income Group` IS NULL THEN 'Other'
      ELSE `Income Group`
    END AS `Income Group`,
    COUNT(*) AS `Number of Countries`
  FROM 
...
grand_total AS (
  SELECT COUNT(*) AS total FROM world_bank_data.wdi_country WHERE `Region` IS NOT NULL
)
SELECT 
  `Income Group`,
  `Number of Countries`,
  (SELECT total FROM grand_total) AS `Total Countries`,
  ROUND(`Number of Countries` * 100.0 / (SELECT total FROM grand_total), 2) AS `Percentage`
FROM 
  income_counts
ORDER BY 
  `Number of Countries` DESC
