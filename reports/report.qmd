...
run_sql_and_return_html(cnx,"""
select
  country_code, short_name, income_group
from 
  world_bank_data.wdi_country
where
  region is null
...
where 
  ...
  or (country_abbr is null and country_wb_abbr is not null)
  or (country_abbr is not null and country_wb_abbr is null)
order by 
  region, short_name
""")
```
...
from 
  wdi_country
where 
  short_name in ('Kuwait', 'Venezuela')
""")
```
...
run_sql_and_return_html(cnx,"""
select 
  wdi_country.country_code, wdi_country.short_name, wdi_country.region, wdi_country.income_group
from 
  wdi_country
where 
...
income_cte as (
  select distinct income_group from wdi_country
),
...
from 
  ...
  left join existing_combinations_cte ec on ac.region = ec.region and ac.income_group = ec.income_group
where 
  ec.region is null
order by 
...
select 
  ...
  round(ric.count / (select total from grand_total) * 100, 1) as "Percent of Total"
from 
...
select 
  region,
  round(sum(case when income_group = 'High income' then count else 0 end) / (select total from grand_total) * 100, 1) as "High income %",
  round(sum(case when income_group = 'Upper middle income' then count else 0 end) / (select total from grand_total) * 100, 1) as "Upper middle income %",
  round(sum(case when income_group = 'Lower middle income' then count else 0 end) / (select total from grand_total) * 100, 1) as "Lower middle income %",
  round(sum(case when income_group = 'Low income' then count else 0 end) / (select total from grand_total) * 100, 1) as "Low income %",
  round(sum(count) / (select total from grand_total) * 100, 1) as "Region % of Total"
from 
  region_income_counts
group by 
  region
...
select 
  ...
  round(ic.count / (select total from grand_total) * 100, 1) as "Percent of Total"
from 
  income_counts ic
order by 
  ic.count desc
