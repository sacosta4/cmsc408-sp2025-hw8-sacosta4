...
where
  ...
  and `Short Name` <> 'Marshall Islands'
order by
  `Short Name`
""")
...
with all_pairs as (
  ...
  cross join
    (select distinct `Income Group` from world_bank_data.wdi_country) i
),
existing_pairs as (
  select distinct
    `Region`,
...
select
  c.`Region`,
  round(sum(case when `Income Group` = 'High income' then num_countries else 0 end) * 100.0 / g.total_countries, 2) as `High income`,
  round(sum(case when `Income Group` = 'Upper middle income' then num_countries else 0 end) * 100.0 / g.total_countries, 2) as `Upper middle income`,
  round(sum(case when `Income Group` = 'Lower middle income' then num_countries else 0 end) * 100.0 / g.total_countries, 2) as `Lower middle income`,
  round(sum(case when `Income Group` = 'Low income' then num_countries else 0 end) * 100.0 / g.total_countries, 2) as `Low income`,
  round(sum(case when `Income Group` is null then num_countries else 0 end) * 100.0 / g.total_countries, 2) as `Other`,
  round(sum(num_countries) * 100.0 / g.total_countries, 2) as `Total`
from
  counts c
cross join
  grand_total g
